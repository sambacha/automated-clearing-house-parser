package com.freighttrustachViewer.ach;
...
public class ACHFile {
  ...
  public void parseFile(String filename) throws Exception {
    ...
    try {
      ...
      throw new Exception(
          "File " + achFile.getPath() + " could not be opened. Reason " + ex.getMessage());
    }
    parse(achReader);
  }
  ...
  private void parse(BufferedReader achReader) throws Exception {
    ...
    try {
      ...
      if (!record.substring(0, 1).equals("1")) {
        throw new Exception("Content is not an ACH format.  First character must be a \"1\"");
      }
      ...
      while (record != null) {
        ...
        for (int recStart = 0; recStart < record.length(); recStart += recLength) {
          ...
          ACHRecord achRecord = ACHRecord.parseACHRecord(record.substring(recStart, endRec));
          if (achRecord.isFileHeaderType()) {
            setFileHeader((ACHRecordFileHeader) achRecord);
            recLength = Integer.parseInt(getFileHeader().getRecordSize());
            achEntry = null;
            achBatch = null;
            ...
            achBatch.setBatchHeader((ACHRecordBatchHeader) achRecord);
            ...
            achBatch.setBatchControl((ACHRecordBatchControl) achRecord);
            ...
            achEntry.setEntryDetail((ACHRecordEntryDetail) achRecord);
  ...
  public BatchTotals getBatchTotals() throws Exception {
    ...
    try {
      ...
      for (int i = 0; i < getBatches().size(); i++) {
        ...
        // block count
        retValue.recordCount +=
            Long.parseLong(getBatches().get(i).getBatchControl().getEntryAddendaCount()) + 2;

        retValue.entryAddendaCount +=
            Long.parseLong(getBatches().get(i).getBatchControl().getEntryAddendaCount());
        retValue.entryHash += Long.parseLong(getBatches().get(i).getBatchControl().getEntryHash());
        retValue.creditDollarAmount +=
            Long.parseLong(getBatches().get(i).getBatchControl().getTotCreditDollarAmt());
        retValue.debitDollarAmount +=
            Long.parseLong(getBatches().get(i).getBatchControl().getTotDebitDollarAmt());
      }
      // Restrict hash to the first 10 characters

      retValue.entryHash =
          Long.parseLong(
              ACHRecord.formatACHDecimal(String.valueOf(retValue.entryHash), "0000000000"));
    } catch (Exception ex) {
      throw ex;
    }
  ...
  public boolean recalculate() {
    ...
    try {
      ...
      fileControl.setEntryAddendaCount(String.valueOf(totals.entryAddendaCount));
      fileControl.setTotDebitDollarAmt(String.valueOf(totals.debitDollarAmount));
      fileControl.setTotCreditDollarAmt(String.valueOf(totals.creditDollarAmount));
  ...
  public Vector<String> validate() {
    ...
    try {
      ...
      long entryAddendaCount = Long.parseLong(fileControl.getEntryAddendaCount());
      long entryDebitDollarAmount = Long.parseLong(fileControl.getTotDebitDollarAmt());
      long entryCreditDollarAmount = Long.parseLong(fileControl.getTotCreditDollarAmt());
      ...
      if (totals.entryAddendaCount != entryAddendaCount) {
        retvalue.add(
            "File count is out of balance -- file: "
                + entryAddendaCount
                + "  Calced: "
      ...
      if (entryDebitDollarAmount != totals.debitDollarAmount) {
        retvalue.add(
            "File debits are out of balance -- file: "
                + entryDebitDollarAmount
                + "  Calced: "
      ...
      if (entryCreditDollarAmount != totals.creditDollarAmount) {
        retvalue.add(
            "File credits are out of balance -- file: "
                + entryCreditDollarAmount
                + "  Calced: "
      ...
      if (entryHash != totals.entryHash) {
        retvalue.add(
            "File hash is out of balance -- file: " + entryHash + "  Calced: " + totals.entryHash);
        ;
      }
      if (batchCount != totals.batchCount) {
        retvalue.add(
            "File batch count is out of balance -- file: "
                + batchCount
                + "  Calced: "
                + totals.batchCount);
        ;
      ...
      if (blockCount != calcBlockCount) {
        retvalue.add(
            "File block count is out of balance -- file: "
                + blockCount
                + "  Calced: "
                + calcBlockCount);
        ;
  ...
  public boolean save(String filename) throws Exception {
    ...
    for (int i = 0; i < batches.size(); i++) {
      ...
      for (int j = 0; j < achEntries.size(); j++) {
        ...
        rowCount++;
        // Output each addenda
        Vector<ACHRecordAddenda> achAddendas = achEntries.get(j).getAddendaRecs();
    ...
    try {
      int blockSize = Integer.parseInt(getFileHeader().getBlockingFactor());
      int neededRows = blockSize - (rowCount % blockSize);
      if (neededRows > 0) {
        int recordSize = Integer.parseInt(getFileHeader().getRecordSize());
      ...
      System.err.println("Unable to output trailing control records -- " + ex.getMessage());
      ex.printStackTrace();
    }
