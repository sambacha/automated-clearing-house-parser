...
public class ACHBatch {
  ...
  /** @param batchControl the batchControl to set */
  ...
  public EntryAddendaTotals getEntryTotals() throws Exception {
    ...
    try {
      ...
      for (int i = 0; i < entryRecs.size(); i++) {
        retValue.entryAddendaCount += entryRecs.get(i).getAddendaRecs().size();
        retValue.entryHash += Long.parseLong(entryRecs.get(i).getEntryDetail().getReceivingDfiId());
        if (entryRecs.get(i).getEntryDetail().getTransactionCode().charAt(1) <= '4') {
          retValue.creditDollarAmount +=
              Long.parseLong(entryRecs.get(i).getEntryDetail().getAmount());
        } else {
  ...
  public boolean reverse() {
    for (int i = 0; i < entryRecs.size(); i++) {
      try {
        ...
        System.err.println(
            "Unable to reverse entry in batch "
                + getBatchHeader().getBatchNumber()
                + " Entry number "
                + i
                + " Entry trace "
  ...
  public boolean recalc() {
    ...
    try {
      ...
      getBatchControl().setEntryAddendaCount(String.valueOf(totals.entryAddendaCount));
      getBatchControl().setTotDebitDollarAmt(String.valueOf(totals.debitDollarAmount));
      getBatchControl().setTotCreditDollarAmt(String.valueOf(totals.creditDollarAmount));
  ...
  public Vector<String> validate() {
    ...
    try {
      try {
        entryAddendaCount = Long.parseLong(batchControl.getEntryAddendaCount());
        entryDebitDollarAmount = Long.parseLong(batchControl.getTotDebitDollarAmt());
        entryCreditDollarAmount = Long.parseLong(batchControl.getTotCreditDollarAmt());
        entryHash = Long.parseLong(batchControl.getEntryHash());
      } catch (Exception ex) {
        retvalue.add(
            "Batch number "
                + batchHeader.getBatchNumber()
      ...
      long truncatedHash =
          Long.parseLong(
              ACHRecord.formatACHDecimal(String.valueOf(totals.entryHash), "0000000000"));
      if (entryHash != truncatedHash) {
        retvalue.add(
            "Batch "
                + batchHeader.getBatchNumber()
                + " hash is out of balance -- batch: "
                + entryHash
                + "  Calced: "
                + truncatedHash);
        ;
      ...
      ex.printStackTrace();
      retvalue.add(
          "Error processing batch " + batchHeader.getBatchNumber() + " : " + ex.getMessage());
    }
    return retvalue;
  }
  ...
  /** @param batchHeader the batchHeader to set */
